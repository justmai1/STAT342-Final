...
Based off this sample, we can see that the vaccine is effective in preventing infection of COVID-19 in comparison to the placebo. However, to truly test this, we will use both a likelihood inference and bayesian inference to confirm this result.
...
```{r, echo=F, label="taylor_plot", warning=F, fig.height=4, fig.width=7}
...
We will then estimate our $\psi$ value based off prior and posterior observations and find the credible interval for $\psi$. Then, we've test $H_0: \psi = 0.3, H_a: \psi > 0.3$ `pbeta()`. We will then analyze our results in comparison to the frequentist approach and the given report.
...
**Bayesian Approach** Now for our own inference, we will test the vaccine efficacy and credible interval to compare our results to Pfizer's analysis by utilizing the prior and posterior median using a prior of $Beta(2, 10)$. To initialize our Bayesian Inference, we will be analyzing the prior assumption, to see the prior probability of receiving an infection in the vaccine group (knowledge of the sample isn't involved).

Figure 3:

```{r label="prior_dist", eval=TRUE, echo=FALSE, fig.height=4, fig.width=4, fig.align='center', message=FALSE}
# Figure 5: prior distribution
pi_values <- seq(0,1,length.out=1000)
prior_values <- dbeta(pi_values, shape1 = 2, shape2 = 100)
...
prior_df %>% 
  ggplot(aes(pi_values, prior_values)) + 
    geom_line() +
    geom_vline(xintercept = qbeta(0.5, shape1=2, shape2 = 100), linetype="dotted") +
    labs(title = "Prior Distribution (Given)")

cat("Probabiliy of receiving an infection in vaccine group:", 
    qbeta(0.5, shape1=2, shape2 = 100))
```

We will be looking at the median infection probability for the central estimate. This demonstrates that our prior assumption suggests that there is a `r qbeta(0.5, shape1=2, shape2 = 10)` probability of receiving an infection in the vaccine group (based off the median infection probability).

We now observe that in the treatment group, there were 8 infections our of a sample size of 170 (number of total cases). This sets us up for us to find our $Beta(\alpha_{posterior}, \beta_{posterior})$ which is $Beta(\alpha_{prior} + X_V, n - X_V + \beta_{posterior})$ (Chapter 13.2, Slide 8)

Figure 4:

```{r label="post_dist", eval=TRUE, echo=FALSE, fig.height=4, fig.width=7, fig.align='center', message=FALSE}
# Figure 4: prior and posterior distribution
posterior_values <- dbeta(pi_values, shape1=2+8, shape2 = 170-8+100)
...
pi_v_lower <- qbeta(0.025, shape1=2+8, shape2 = 170-8+100)
pi_v_upper <- qbeta(0.975, shape1=2+8, shape2 = 170-8+100)
...
```{r, label="emp_0.3" ,echo=F, fig.width=6, fig.height=4}
...
Our estimation for efficacy based off the Bayesian approach using the posterior median of `r pi_v_median` would be `r efficacy`. We found that our 95% posterior interval comes out to `r qbeta(c(0.025,0.975), shape1=2+8, shape2 = 170-8+10)`. We then apply these values to get the credible interval for efficacy between `r efficacy_lower` and `r efficacy_upper`

After conducting a hypothesis test to test $H_0: \psi = 0.3, H_a: \psi > 0.3$, we found that the posterior probability of $\psi$ being greater than 0.3 is very high (low p-value).
...
cat("Given the efficacy is 30%, what is the likelihood of seeing this value:", 
    1-pbeta(7/17, shape1=2+8, shape2 = 170-8+100))
```
...
Based on the results of our analysis, we found that this vaccine meets FDA efficacy standards and should be approved for rollout. According to our confidence intervals, the vaccine should at the minimum have 93% efficacy, with the most likely efficacy based on the study being about 95%, for both the frequentist and Bayesian model. Thus, this study has disproved/approved [WHAT IS OUR HYPOTHESIS] the hypothesis that ____. 
Using the frequentist approach based on the data from the study, we found that the 95% confidence interval for vaccine efficacy is [0.9479, 0.9533]. Using a bootstrapped sample from this data provided a similar interval of [0.9494, 0.9516]. The Bayesian approach provided more broad intervals of [0.9338, 0.9818]. This is partially due to the Bayesian approach’s reliance on a prior distribution, the choosing of which can have a significant effect on the results. Pfizer’s analysis chose a relatively weak prior, getting them a credible interval of [90.3%, 97.6%]. We decided to perform our analysis with a stronger prior, strongly believing that the vaccine is effective in preventing infection. Our prior yields a much greater and tighter credible interval in comparison, which further demonstrates our strong belief in the vaccine. Even though the Bayesian approach's credible interval is wider than the frequentist's, all of these confidence intervals support __ this hypothesis because all efficacy rates contained in these intervals are beyond the 0.3 standard set by the FDA. Additionally we conducted a p-value calculation against the null hypothesis of an efficacy rate below 0.3, and both frequentist p-values were approximately 0, meaning that it was nearly impossible to achieve the statistics found in the study if the vaccine’s true efficacy was below 0.3. The Bayesian approach found a similar result, with a p-value of __. It is on the basis of this data that we __ the hypothesis that this vaccine’s efficacy is greater than the FDA’s standard of 0.3.

...
In 2020, when the Coronavirus was at its highest in terms of world impact, scientists were attempting to develop vaccines to reduce the number of COVID-19 cases. To test this, researchers randomly appointed candidates of 16 years old or older to either the placebo dosage or the vaccine dosage called BNT162b2, the ratio of placebo vs. vaccine is 1:1. Both parties (doctors and patients) were unaware of which type of they were given/taken to avoid confirmation bias, known as double-blind trials. Each research group were given 2 dosages, 21 days apart, of their respective shots. Then, after at least 7 days since the candidate receives the 2nd dosage, they were tested to see if they had been infected with COVID-19. They stopped conducting this experiment after **170 total infection cases** were seen.

By using the double-blind trials to leave the type of treatment unknown to both the doctor and the patient, it will remove an idea of "suggestion" to avoid subconscious biases. Generally, for drugs such as the BNT162b2 vaccine to be approved by governments and organizations, even though it takes time, it is a necessary research step to improve and test vaccines (Ryder).
...
vac_df_wide <- data.frame(BNT162b2 = c(8, 17411), 
                          Placebo = c(162, 17511), 
                          Total = c(170,34922))
...
Based off this sample, we can see that the vaccine is effective in preventing infection of COVID-19 in comparison to the placebo. However, to truly test this, we will use both a likelihood inference and baysian inference to confirm this result.
...
Given $\pi = \frac{\pi_V}{\pi_V + \pi_p}$ where $\pi_V$ and $\pi_p$ are the probabilities of an infection in the vaccine and placebo groups respectively.
...
```{r, echo=F, label="taylor_plot", warning=F}
...
We know $L(\pi)$ from our binomial distribution for T:


$L(\pi)=\binom{n}{T}\pi^T(1-\pi)^{n-T}$


$L^*(\psi)=\binom{n}{T}(\frac{\psi-1}{\psi-2})^T(1-\frac{\psi-1}{\psi-2})^{n-T}$ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ def of $\pi$ \ \ \ \ \ \ \ \ NOTE: $\psi$ or $\psi^*$???????????????? 
...
$\ell^*(\psi)=ln(\binom{n}{T}(\frac{\psi-1}{\psi-2})^T(1-\frac{\psi-1}{\psi-2})^{n-T})$



$\psi_0^{MLE}=\frac{(n - 2T)}{(n-T)}$  
...
We have $\hat\psi^{mle}$ = 0.9506173

So W =  2ln($L(0.9506173)/L(0.3)$) \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ $\psi^{null}= 0.3$
...
We can now use the fact that W is a chi square distribution with degree of freedom = 1.
...
Because we clearly were able to reject for $\psi=0.3$, we wanted to try a harsher test within the context of the FDA requiring even more efficacy specifically for COVID-19 vaccines.
...
Within the study, it was stated that the prior assumption for parameter $\pi$ is modeled with $Beta(0700102, 1)$ prior. Also, the vaccine efficacy used is $\psi = \frac{1-2\pi}{1-\pi}$ was estimated to be 95%, this means that by using the Bayesian Inference, they deduced that the vaccine reduces the probability of infection by 95% when compared to the placebo group. Also, the Bayesian credible interval was reported to be [90.3%, 97.6%]. We'll now conduct the Baysian inference to test these values

*Prior Distribution*: The data model is stated to be a binomial model (vaccine or placebo) for the prior distribution, which is $T_\pi \sim Binom(170, \pi)$ where $\pi$ is our probability of success P(vaccine|infection).
...
We are interested in $\psi = \frac{1-2\pi}{1-\pi}$ which is looking at how much the vaccine reduces the risk of infection compared to the placebo. We are also interested in the credibal interval for $\psi$ and are testing $H_0: \psi = 0.3, H_a: \psi > 0.3$

*Approach*: To approach this inference, we will first be retesting the $\psi$ value given by the paper of 95% to confirm the results. We will then conduct our own Bayesian inference by retrieving the prior and posterior median as our $\pi$ value. We've decided to take this approach because we believe that the probability of receiving an infection with the vaccine is low, meaning we believe the prior and posterior distribution will be strongly right skewed, hence we are using the median instead of the mean because of the impact skewness has on the mean. 

We will then estimate our $\psi$ value based off prior and posterior observations and find the credible interval for $\psi$. Then, we've test $H_0: \psi = 0.3, H_a: \psi > 0.3$ to find the posterior probability that vaccine efficacy exceeds 30% using `pbeta()`. We will then analyze our results in comparison to the frequentist approach and the given report.



## Bayesian Inference

For the first part of our Bayesian approach, we are retesting the approach taken within the research paper then comparing our vaccine efficacy and the credible interval to the one published by the paper.

Sample: Vaccine Group ($X_V = 8$ cases), Placebo Group ($X_p = 162$ cases) out of a total of $n = 170$ observed cases.

Firstly, we will retest their values to ensure that the conclusion they received was correct (or about correct). Given the sample, the probability of $\pi_V = \frac{8}{17411}$ and the probability of $\pi_p = \frac{162}{17511}$. Using these values, we can get an estimate probability of $$\pi = \frac{\frac{8}{17411}}{\frac{8}{17411} + \frac{162}{17511}} = 0.0473$$ Now we can estimate the vaccine efficacy $\psi = \frac{1- 2\pi}{1-\pi} = 0.95035$ which matches the estimate of 95% given by the report.

**Bayesian Approach** Now for our own inference, we will retest the vaccine efficacy and credible interval by utilizing the prior and posterior median. To initialize our Bayesian Inference, we will be analyzing the prior assumption, to see the prior probability of receiving an infection in the vaccine group (knowledge of the sample isn't involved).

```{r label="prior_dist", eval=TRUE, echo=FALSE, fig.height=4, fig.width=4, fig.align='center'}
pi_values <- seq(0,1,length.out=1000)
prior_values <- dbeta(pi_values, shape1 = 0.700102, shape2 = 1)
...
cat("Probabiliy of receiving an infection in vaccine group:", 
    qbeta(0.5, shape1=0.700102, shape2 = 1))
```

We will be looking at the median infection probability for the central estimate. This demonstrates that our prior assumption suggests that there is a `r qbeta(0.5, shape1=0.700102, shape2 = 1)` probability of receiving an infection in the vaccine group (based off the median infection probability).

We now observe that in the treatment group, there were 8 infections our of a sample size of 170 (number of total cases). This sets us up for us to find our $Beta(\alpha_{posterior}, \beta_{posterior})$ which is $Beta(\alpha_{prior} + X_V, n - X_V + \beta_{posterior})$

```{r label="post_dist", eval=TRUE, echo=FALSE, fig.height=4, fig.width=7, fig.align='center'}
posterior_values <- dbeta(pi_values, shape1=0.700102+8, shape2 = 170-8+1)
...
boot_df %>% summarise(mean = mean(psihat),
                      sd =sd(psihat))
```
...
P-value = $2.822313*10^{-28}$



```{r, label="emp_0.3" ,echo=F}
...
empirical_p_value <- mean(W_sim >= W_obs)
...
Our empirical p-value equals 0 for efficacy 0.3


Bayesian Confidence Interval

Our 95% Credible Interval: Between `r efficacy_lower` and `r efficacy_upper`



```{r label="post_prob_30", eval=TRUE, echo=FALSE}
# Finding p-value using posterior mean
cat("Posterior probability that vaccine efficacy exceeds 30%:", 
    pbeta(7/17, shape1=0.700102+8, shape2 = 170-8+1))
```
...
results <- data.frame(
  ...
  Confidence_Interval_Lower = c(0.9479, 0.8070398),
  Confidence_Interval_Higher = c(0.9533, 0.9525105),
  P_value = c("2.822313e-28", 0)
)

# Print table using kable for basic formatting
knitr::kable(results, caption = "Results Table")
...
Based on the results of our analysis, we found that this vaccine meets FDA efficacy standards and should be approved for rollout. According to our confidence intervals, the vaccine should at the minimum have 80% efficacy, with the most likely efficacy based on the study being 90 or 95%, depending on the choice between frequentist and Bayesian model. Thus, this study has disproved/approved [WHAT IS OUR HYPOTHESIS] the hypothesis that ____. 
Using the frequentist approach based on the data from the study, we found that the 95% confidence interval for vaccine efficacy is [0.9479, 0.9533]. Using a bootstrapped sample from this data provided a similar interval of [0.9494, 0.9516]. The Bayesian approach provided more broad intervals of [0.807, 0.9525]. This is partially due to the Bayesian approach’s reliance on a prior distribution, the choosing of which can have a significant effect on the results. Pfizer’s analysis chose a relatively weak prior, while we performed an additional analysis with a more pessimistic prior distribution with respect to the efficacy in order to prove this vaccine’s efficacy beyond a reasonable doubt. Even so, all of these confidence intervals support __ this hypothesis because all efficacy rates contained in these intervals are beyond the 0.3 standard set by the FDA. Additionally we conducted a p-value calculation against the null hypothesis of an efficacy rate below 0.3, and both frequentist p-values were approximately 0, meaning that it was nearly impossible to achieve the statistics found in the study if the vaccine’s true efficacy was below 0.3. The Bayesian approach found a similar result, with a p-value of __. It is on the basis of this data that we __ the hypothesis that this vaccine’s efficacy is greater than the FDA’s standard of 0.3.


Bayesian

Because our p-value is equal to `r pbeta(7/17, shape1=0.700102+8, shape2 = 170-8+1)`, we can comfortably reject the null hypothesis that $\psi = 0.3$. This demonstrates that our result is statistically significant, suggesting that our vaccine efficacy is much higher than 0.3.
...
$0=(T/(\psi-1) - T/(\psi-2)+\frac{d}{d\psi}((n-T)(-ln(2-\psi))))$
...
```{r ref.label = "post_prob_30", eval=FALSE}
```
